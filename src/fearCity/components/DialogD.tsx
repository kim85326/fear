...
import DialogDText from "./DialogDText";

interface IDialogDProps {
  isShowDialog: boolean;
  dialogX: number;
  dialogY: number;
}

interface IDialogDState {
  isShowDialog: boolean;
...
class DialogD extends React.Component<IDialogDProps, IDialogDState> {
  constructor(props: IDialogDProps) {
    super(props);
    this.state = {
      isShowDialog: false,
  ...
  public componentWillReceiveProps(nextProps: IDialogDProps) {
    // 當 props 改變，且顯示對話框時
  ...
  public render() {
    return (
      <div
        className={`dialogD ${this.state.isShowDialog ? "show" : ""}`}
        style={{ top: this.props.dialogY, left: this.props.dialogX }}
      >
        <DialogDText isShowDialogText={this.state.isShowDialogText} />
        <svg
          className={`dialogD-background 
