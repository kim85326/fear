...
import DialogEText from "./DialogEText";

interface IDialogEProps {
  isShowDialog: boolean;
  dialogX: number;
  dialogY: number;
}

interface IDialogEState {
  isShowDialog: boolean;
...
class DialogE extends React.Component<IDialogEProps, IDialogEState> {
  constructor(props: IDialogEProps) {
    super(props);
    this.state = {
      isShowDialog: false,
  ...
  public componentWillReceiveProps(nextProps: IDialogEProps) {
    // 當 props 改變，且顯示對話框時
  ...
  public render() {
    return (
      <div
        className={`dialogE ${this.state.isShowDialog ? "show" : ""}`}
        style={{ top: this.props.dialogY, left: this.props.dialogX }}
      >
        <DialogEText isShowDialogText={this.state.isShowDialogText} />
        <svg
          className={`dialogE-background 
