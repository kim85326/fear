...
interface IButtonAState {
  isShowButton: boolean;
  isShowingButtonBackground: boolean;
  isShowButtonText: boolean;
}

class ButtonA extends React.Component<IButtonAProps, IButtonAState> {
  constructor(props: IButtonAProps) {
    super(props);
  ...
  public componentWillReceiveProps(nextProps: IButtonAProps) {
    // 當 props 改變，且顯示按鈕時
  ...
  public render() {
    return (
      <Link
        to="/storyA"
        className={`buttonA ${this.state.isShowButton ? "show" : ""}`}
      >
