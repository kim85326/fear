...
import DialogGText from "./DialogGText";

interface IDialogGProps {
  isShowDialog: boolean;
}

interface IDialogGState {
  isShowDialog: boolean;
...
class DialogG extends React.Component<IDialogGProps, IDialogGState> {
  constructor(props: IDialogGProps) {
    super(props);
  ...
  public componentWillReceiveProps(nextProps: IDialogGProps) {
    // 當 props 改變，且顯示對話框時
  ...
  public render() {
    return (
      <div className={`dialogG ${this.state.isShowDialog ? "show" : ""}`}>
        <DialogGText isShowDialogText={this.state.isShowDialogText} />
        <svg
          className={`dialogG-background
